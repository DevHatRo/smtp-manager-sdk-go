...
//
// and optionally SMTP_MANAGER_BASE_URL for a self-hosted instance (the
// hosted platform, smtpmanager.DefaultBaseURL, is used when it is unset).
//
...
func TestIntegration_Send(t *testing.T) {
	...
	if apiKey == "" || from == "" || to == "" {
		t.Skip("set SMTP_MANAGER_API_KEY, SMTP_MANAGER_FROM and SMTP_MANAGER_TO to run (SMTP_MANAGER_BASE_URL optional)")
	}
	...
	var opts []smtpmanager.Option
	if baseURL != "" {
		opts = append(opts, smtpmanager.WithBaseURL(baseURL))
	}
	client, err := smtpmanager.New(apiKey, opts...)
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	t.Logf("sending through %s", client.Endpoint())
