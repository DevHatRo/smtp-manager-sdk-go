...
func main() {
    ...
    // created (https://host/api/v1/messages) or just the API origin.
...
|---|---|---|
| `WithBaseURL(url)` | — (required) | The Endpoint the console showed (`https://smtp.example.com/api/v1/messages`) or the API origin (`https://smtp.example.com`); `/api`, `/api/v1` and trailing slashes are understood too. `client.Endpoint()` returns the resolved URL. A URL carrying `user:pass@` is refused: the API key is the only credential. |
...
your secret manager and pass it to `New`; pass the Endpoint the console
showed, or the API origin, to `WithBaseURL` — both resolve to the same URL.
The key must have the `send` ability, otherwise every call returns
`CodePermission`.
...
skips itself unless all four variables are set:

```bash
export SMTP_MANAGER_BASE_URL=https://smtp.example.com
...
In CI the integration job is `workflow_dispatch` only (Actions → CI → Run
workflow) and reads the same four values from repository secrets; it never
runs on pushes or pull requests, because it sends real mail.
